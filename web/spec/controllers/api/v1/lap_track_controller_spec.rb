...
RSpec.describe Api::V1::LapTrackController, :type => :controller do
  describe "track lap" do
    ...
    it "simulate a race in competition mode" ,:type => :request do
      ...
      expect(response.status).to eq 200
      assert_equal 1, RacePilotPlacementAnnouncerWorker.jobs.size # should play a sound file
      ...
      expect(response.status).to eq 200
      assert_equal 2, RacePilotPlacementAnnouncerWorker.jobs.size # should play a sound file

      post '/api/v1/lap_track',transponder_token: 10, lap_time_in_ms: 13000
      expect(response.status).to eq 200
      assert_equal 3, RacePilotPlacementAnnouncerWorker.jobs.size # should play a sound file
      RacePilotPlacementAnnouncerWorker.drain
      assert_equal 0, RacePilotPlacementAnnouncerWorker.jobs.size # should play a sound file
    ...
    it "simulate a race in competition mode with hot seat mode" ,:type => :request do
      ConfigValue::set_value("time_between_lap_track_requests_in_seconds",4)
      ConfigValue::set_value("lap_max_lap_time_in_seconds",60)

      Timecop.return
      # setting up the race data
      pilot_1 = Pilot.create(name: "Pilot 1", transponder_token: 10, quad:'ZMR')
      pilot_2 = Pilot.create(name: "Pilot 2", transponder_token: 20, quad:'ZMR')
      pilot_3 = Pilot.create(name: "Pilot 3", transponder_token: 30, quad:'ZMR')

      new_race_data = Hash.new
      new_race_data[:title] = "new competition race"
      new_race_data[:max_laps] = 3
      new_race_data[:pilots] = Array.new
      new_race_data[:hot_seat_enabled] = true # enabling hot seat mode

      # we don't need to add pilots in competition mode with hot sead enabled, the system should add them by itself
      #new_race_data[:pilots] << {pilot_id: pilot_1.id, transponder_token: 10}
      #new_race_data[:pilots] << {pilot_id: pilot_2.id, transponder_token: 20}
      #new_race_data[:pilots] << {pilot_id: pilot_3.id, transponder_token: 30}
      ...
      expect(open_race_session.mode).to eq("competition")
      expect(open_race_session.active).to eq(true)
      expect(open_race_session.hot_seat_enabled).to eq(true)
